...
def parse_file(filename, target_chains, **kwargs):
    """ Parse a PDB file and return a list of dictionaries with sequence and coordinates """
    return parse_traj(md.load(filename), target_chains, filename=filename, **kwargs)

def parse_traj(traj, target_chains, **kwargs):

    top = kwargs.get('top', traj.topology)
    ca_only = kwargs.get('ca_only', False)
    traj_file = kwargs.get('filename', "traj_object")

    # temp fix for mdtraj bugs
    for cc, c in zip(top.chains, target_chains):
        cc.chain_id = c

    n_frames = traj.n_frames

    # "num_of_chains" field
    chains = [c for c in top.chains if c.chain_id in target_chains]
    num_of_chains = len(chains)
    if num_of_chains != len(target_chains):
        raise ValueError(f"Some chains are missing in coordinate file.")
    
    atoms = ['CA'] if ca_only else ['N', 'CA', 'C', 'O']
    indices = {c: {a: [] for a in atoms} for c in target_chains}
    seq = {c: "" for c in target_chains}

    # deal with topology
    for c in chains:
        resid = [res.resSeq for res in c.residues]
        for a in atoms:
            indices[c.chain_id][a] = [-1 for _ in range(len(resid))]
        for ii, i in enumerate(range(min(resid), max(resid) + 1)):
            try:
                res = c.residue(i - 1)
                code = res.code
                for a in atoms:
                    indices[c.chain_id][a][ii] = list(res.atoms_by_name(a))[0].index
            except IndexError:
                code = "-"
            finally:
                seq[c.chain_id] += code
    ...
    template = {
        "seq": "".join([seq[c] for c in target_chains]),
        "name": pathlib.Path(traj_file).stem + ".frame_{:.2f}",
        "num_of_chains": num_of_chains
    }
    for c in chains:
        template[f"seq_chain_{c.chain_id}"] = seq[c.chain_id]

    return [parse_frame(traj.slice(i, copy=False), indices, template) for i in range(n_frames)]

def parse_frame(traj, indices, template):

    frame = {k: template[k] for k in template}
    frame["name"] = frame["name"].format(traj.time[0])

    for c in indices:
        frame[f"coords_chain_{c}"] = {
            f"{a}_chain_{c}": [
                (traj.xyz[0][i]*10).tolist() if i != -1 else np.full((3,), np.nan)
                for i in indices[c][a]
            ] for a in indices[c]
        }

    return frame    

def tied_featurize(batch, device, all_chains, ca_only=False):
    ...
    for i, b in enumerate(batch):
        ...
        chain_seq = [b[f'seq_chain_{c}'].replace('X', '-') for c in all_chains]
        chain_length = [len(c) for c in chain_seq]
        cumsum = np.append(0, np.cumsum(chain_length))

        fixed_position_mask_list = []
        for c, letter in enumerate(all_chains):

            chain_coords = b[f'coords_chain_{letter}'] # this is a dictionary
            keys = ['CA_chain'] if ca_only else ['N_chain', 'CA_chain', 'C_chain', 'O_chain']              
            x_chain_list.append(np.stack([chain_coords[f"{k}_{letter}"] for k in keys], axis=1))

            chain_mask_list.append(np.ones(chain_length[c])) # 1.0 for masked
            chain_encoding_list.append(np.full(chain_length[c], c + 1))
            fixed_position_mask_list.append(np.ones(chain_length[c]))

            begin, end = cumsum[c], cumsum[c + 1]
            residue_idx[i, begin:end] = 100 * c + np.arange(begin, end)

        x = np.concatenate(x_chain_list, axis=0) #[L, 4, 3]
        m = np.concatenate(chain_mask_list, axis=0) #[L,], 1.0 for places that need to be predicted
        chain_encoding = np.concatenate(chain_encoding_list, axis=0)
        m_pos = np.concatenate(fixed_position_mask_list, axis=0) #[L,], 1.0 for places that need to be predicted

        all_sequence = "".join(chain_seq)
        l = len(all_sequence)
        X[i,:,:,:] = np.pad(x, [[0, L_max - l], [0,0], [0,0]], 'constant', constant_values=(np.nan, ))
        chain_M[i,:] = np.pad(m, [[0, L_max - l]], 'constant', constant_values=(0.0, ))
        chain_M_pos[i,:] = np.pad(m_pos, [[0, L_max - l]], 'constant', constant_values=(0.0, ))
...
class ProteinMPNN(nn.Module):
    ...
    def forward(self, X, S, mask, chain_M, residue_idx, chain_encoding_all, randn, decoding_order=None):
        ...
        if decoding_order is None:
            ...
            # fix decoding order
            decoding_order = torch.arange(chain_M.shape[1], device=device).unsqueeze(0).expand_as(chain_M)
